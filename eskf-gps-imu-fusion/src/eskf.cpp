...
void ErrorStateKalmanFilter::ComputeOrientation(const Eigen::Vector3d &angular_delta,
    ...
    Eigen::AngleAxisd angle_axisd(angular_delta.norm(), angular_delta.normalized());    // 等效旋转矢量

    last_R = pose_.block<3, 3>(0, 0);
    curr_R = R_nm_nm_1.transpose() * pose_.block<3, 3>(0, 0) * angle_axisd.toRotationMatrix();  // 旋转矩阵
